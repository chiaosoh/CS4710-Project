...
class WorldsHardestGameEnv(gym.Env):
    def __init__(self, grid_size=10, num_dots=3, max_time_steps=100):
        ...
        self.observation_space = spaces.Box(low=0, high=self.grid_size-1, shape=(self.num_dots + 2 + 1 + len(self.walls), 2), dtype=np.float32)
        self.reset()
        

    def _generate_walls(self):
        # Generate walls along the edges of the grid
        walls = [
            np.array([0, 0]), np.array([0, self.grid_size-1]),
            np.array([self.grid_size-1, 0]), np.array([self.grid_size-1, self.grid_size-1])
        ]
    ...
    def step(self, action):
        ...
        for wall in self.walls:
            if np.linalg.norm(self.cube_position - wall) < 1:
                wall_collision = True
                break

        if wall_collision:
            self.cube_position = np.clip(self.cube_position, 1, self.grid_size-2)
            reward -= 0.1  # Penalty for wall collision

        # Corner detection
        corner_threshold = 1.5
        is_in_corner = (self.cube_position[0] < corner_threshold and self.cube_position[1] < corner_threshold) or \
                    (self.cube_position[0] < corner_threshold and self.cube_position[1] > self.grid_size - corner_threshold) or \
                    (self.cube_position[0] > self.grid_size - corner_threshold and self.cube_position[1] < corner_threshold) or \
                    (self.cube_position[0] > self.grid_size - corner_threshold and self.cube_position[1] > self.grid_size - corner_threshold)

        if is_in_corner:
            reward -= 0.2  # Small penalty for being in a corner
        # Goal check
...
if __name__ == '__main__':
    ...
    model = PPO("MlpPolicy", env, learning_rate=3e-4, n_steps=2048, batch_size=64, n_epochs=10, gamma=0.995, clip_range=0.2, ent_coef=0.01, verbose=1)

    total_timesteps = 0
    max_total_timesteps = 20_000 # Increased training time

    while total_timesteps < max_total_timesteps:
        model.learn(total_timesteps=2500)  # Increased learning steps
        total_timesteps += 2500

        eval_env = WorldsHardestGameEnv(grid_size=10, num_dots=3)
        mean_reward, std_reward = evaluate_policy(model, eval_env, n_eval_episodes=20)  # Increased evaluation episodes
        print(f"Total Timesteps: {total_timesteps}")
        print(f"Mean Reward: {mean_reward:.2f} ± {std_reward:.2f}")

        model.save(f"ppo_worlds_hardest_game_checkpoint_{total_timesteps}")

        if mean_reward > 0.7:  # Increased target reward
